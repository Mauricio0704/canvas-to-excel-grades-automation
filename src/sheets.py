...
def get_rows_of_students(courseID, students_ids):
    ...
    for student_id in students_ids:
        ...
        student_cell = [
            student for student in student_possible_cell if student.row in row_range
        ]

        if student_cell:
            student_rows[student_id] = student_cell[0].row
...
def write_grades_of_assignment(column, grades, student_rows):
    ...
    baseTextFormatDict = {"bold": False, "fontSize": 10, "fontFamily": "Calibri"}
    ...
    for student_id, student_info in grades.items():
        ...
        if not student_row:
            print(
                f"El estudiante con ID: {student_id} no se encontró en la hoja de datos"
            )
            continue
        ...
        if student_info["grade"] == -1:
            ...
            backgroundColorDict = (
                {"red": 1, "green": 0.67843137, "blue": 0.67843137}
                if assignment_col not in [11, 17, 22]
                else {
        ...
        if (
            isinstance(student_info["grade"], (int, float))
            and student_info["grade"] < 50
            and "comment" in student_info
        ):
        ...
        for _ in range(4):
            try:
                service.spreadsheets().batchUpdate(
                    spreadsheetId=SHEET_ID, body={"requests": full_cell_requests}
                ).execute()
