...
def get_students_by_course(course_id):
    return make_canvas_request(
        f"/courses/{course_id}/users",
        params={"type[]": "StudentEnrollment", "status[]": "active", "per_page": 100},
    )


def get_assignments(course_id):
    return make_canvas_request(f"/courses/{course_id}/assignments", {"per_page": 17})


def get_submissions_of_assingment(course_id, assignment_id):
    return make_canvas_request(
        f"/courses/{course_id}/assignments/{assignment_id}/submissions?include[]=submission_comments",
        params={"per_page": 100},
    )
...
def get_grades_of_assignment(course_id, assignment):
    ...
    for submission in submissions:
        if submission["grade"]:
            grade_raw = submission.get("grade")
            grades_dict[submission["user_id"]]["grade"] = parse_grade(grade_raw)
        ...
        if (
            len(submission["submission_comments"]) > 0
            and grades_dict[submission["user_id"]]["grade"] != -1
        ):
            grades_dict[submission["user_id"]]["comment"] = get_tutor_first_comment(
                submission
            )
